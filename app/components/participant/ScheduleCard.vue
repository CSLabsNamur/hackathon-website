...
const nextSpecialEvent = computed<ScheduleItem | null>(() => {
  return props.schedule.find(item => item.special && (item.startTime && item.endTime) && !dayjs(item.startTime).isBefore(mockCurrentDateTime) && mockCurrentDateTime.isAfter(dayjs(item.startTime).subtract(2, "hour"))) || null;
});
</script>
