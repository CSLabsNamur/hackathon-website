...
const fullAddress = computed(() => [
  settings.value!.event.locationName,
...
<template>
  ...
  <UContainer>
    <UPageGrid v-if="settings" class="gap-6 md:gap-8">
      <!-- Emplacement -->
          ...
          <div class="grid gap-4">
            <div class="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20">
              ...
              <div class="flex-1">
                ...
                <p class="text-lg font-semibold text-highlighted">
                  {{ $dayjs(settings.event.startDate).format("DD MMMM YYYY") }}
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-lg font-semibold text-highlighted">
                  {{ $dayjs(settings.event.endDate).format("DD MMMM YYYY") }}</p>
              </div>
            </div>
            ...
            <div class="grid gap-5">
              <div class="flex items-center gap-3 p-3 rounded-lg bg-elevated">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <a :href="`mailto:${settings.website.contactEmail}`"
              ...
              <div class="grid gap-2">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <UButton v-for="link in settings.socialLinks" :key="link.type" :to="link.url" target="_blank"
                           :icon="link.icon" variant="soft" class="flex-1 justify-center"
                           :ui="{base: 'p-2.5'}">
                    {{ link.label }}
                  </UButton>
                </div>
    ...
    <UPageSection v-else title="Informations temporairement indisponibles" icon="i-lucide-info">
      <UAlert color="warning" variant="soft" icon="i-lucide-triangle-alert" title="Détails de l'événement indisponibles"
              description="Les informations pratiques, les dates et les coordonnées ne peuvent pas être chargées pour le moment. Réessayez un peu plus tard."/>
    </UPageSection>
